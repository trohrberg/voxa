...
class StateMachineSkill extends AlexaSkill {
  constructor(config) {
    ...
    this.onLaunchRequest((alexaEvent, reply) => {
      ...
      if (!_.isArray(reply.msg.directives)) {
        reply.msg.directives = [reply.msg.directives];
      }

      // if (reply.msg.directives.length > 0) {
      //   const filtered = StateMachineSkill.filterDirectives(alexaEvent, reply.msg.directives);
      //   reply.msg.directives = filtered;
      // }
  ...
  static filterDirectives(alexaEvent, directives) {
    let interfaces = null;
    if (alexaEvent &&
        alexaEvent.context &&
