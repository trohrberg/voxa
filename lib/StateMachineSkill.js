...
class StateMachineSkill extends AlexaSkill {
  constructor(config) {
    ...
    this.onBeforeReplySent((alexaEvent, reply, transition) => Promise.resolve(alexaEvent.model.serialize())
      .then((modelData) => {
        ...
        if (reply.msg.hasAnAsk) {
          alexaEvent.session.attributes.reply = _.pickBy({ msgPath: transition.reply, state: transition.to.name });
        }
      }));
