...
class StateMachineSkill extends AlexaSkill {
  constructor(config) {
    ...
    this.onBeforeReplySent((alexaEvent, reply, transition) => {
      ...
      // before sending directives check that the directives 
      // are supported by the device. Remove them if they are
      // not supported.
      let interfaces = null;
      if (alexaEvent &&
          alexaEvent.context &&
          alexaEvent.context.System &&
          alexaEvent.context.System.device &&
          alexaEvent.context.System.device.supportedInterfaces) {
        interfaces = alexaEvent.context.System.device.supportedInterfaces;
      } else if (alexaEvent && !alexaEvent.context) {
        // There is a new (beta) view of templates in the developer console.
        // however it does not send any supported directives like real devices.
        // if the context is completely empty we can assume that the request
        // is coming from the developer console and spoof support for all directives.
        interfaces = {
          AudioPlayer: {},
      ...
      if (reply.msg.directives.type) {
        const type = reply.msg.directives.type.split('.');
        if(!interfaces || !interfaces[type[0]]) {
          reply.msg.directives = {};
        }
      }

      // we do require models to have a serialize method and check that when Voxa is initialized,
      // however, developers could do stuff like `alexaEvent.model = null`, which seems natural if they want to
      // clear the model
      if (!serialize) {
    ...
    this.onStateMachineError((alexaEvent, reply, error) => this.handleErrors(alexaEvent, error), true);
  }
      ...
      .then((transition) => {
        ...
        return promise
          .then(() => {
            ...
            return Promise.mapSeries(this.getOnBeforeReplySentHandlers(), fn => fn(alexaEvent, reply, transition));
          })
        .then(() => reply);
      })
