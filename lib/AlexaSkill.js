...
class AlexaSkill {
  ...
  execute(event) {
    ...
    return Promise.try(() => {
      ...
      if (this.config.appIds) {
        if (_.isString(this.config.appIds) && this.config.appIds !== event.context.application.applicationId) {
          debug(`The applicationIds don't match: "${event.context.application.applicationId}"  and  "${this.config.appIds}"`);
          throw new Error('Invalid applicationId');
        }

        if (_.isArray(this.config.appIds) && !_.includes(this.config.appIds, event.context.application.applicationId)) {
          debug(`The applicationIds don't match: "${event.context.application.applicationId}"  and  "${this.config.appIds}"`);
          throw new Error('Invalid applicationId');
        }
      }

      // if click event, set it to IntentRequest to handle
      // like other requests. Set the intent name to the
      // token name. Tokens should be built to reflect the intent
      if (alexaEvent.request.type === 'Display.ElementSelected') {
        alexaEvent.request.type = 'IntentRequest';
        const token = alexaEvent.request.token.split('_');
        const name = token.shift();
        // alexaEvent.request.intent = {
        //   name: name
        // };
        alexaEvent.intent = {
          name: name
        };
        // set the slots to the other data within token
        // example-token: SelectIntent_index:1234_name:cat
        // slots will be set to index: '1234', name: 'cat'
        const slots = {};
        token.forEach(item => {
        ...
        // alexaEvent.request.intent.slots = slots;
        alexaEvent.intent.slots = slots;
      }
   ...
   * they can return a promise if async execution is needed, most are registered with the alexaEvent handlers
