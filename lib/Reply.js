...
class Reply {
  ...
  static toSSML(statement, event) {
    if (event.request.token) {
      // click intents cannot use SSML right now. Remove any markup
  ...
  static wrapSpeech(statement) {
    ...
    let type = "SSML";
    // check for markup
    const xml = /<[a-z][\s\S]*>/i.test(statement);
  ...
  static buildSpeechletResponse(options) {
    ...
    if (options.directives) {
      ...
      } else if (options.directives.type && options.directives.type === 'Display.RenderTemplate') {
