...
class Reply {
  ...
  render() {
    ...
    const reprompt = Reply.wrapSpeech(Reply.toSSML(this.msg.reprompt, this.alexaEvent));
    return { say, reprompt };
  }
  ...
  static compileDirectives(directives) {
    const compiledDirectives = {
      directives:[],
      deleteEndSession: false
    };
    if (!_.isArray(directives)) {
      directives = [directives];
    }
    _.forEach(directives, (d) => {
      if (d.type) {
        if (d.playBehavior) {
          compiledDirectives.directives.push(
            {
              type: d.type,
              ...
              audioItem: {
                stream: {
                  token: d.token,
                  url: d.url,
                  offsetInMilliseconds: d.offsetInMilliseconds,
                }
              }
            }
          )
        } else if (d.type === 'VideoApp.Launch') {
          compiledDirectives.deleteEndSession = true;
        }
        compiledDirectives.directives.push(d);
      }
    });
  ...
  static buildSpeechletResponse(options) {
    ...
    if (options.directives) {
      const compiledDirectives = Reply.compileDirectives(options.directives);
      alexaResponse.directives = compiledDirectives.directives;
      if (compiledDirectives.deleteEndSession) delete alexaResponse.shouldEndSession;
    }
